...
import { useValidationQueue, useValidationReviewItem, useAcceptValidation, useRejectValidation, useFlagValidation } from "@/api/hooks/useValidationAdmin";
  ...
  return (
    <div className="space-y-2">
      {item.categories.map((cat) => (
        <div key={cat.category} className="border border-[var(--border)] rounded-md p-3 flex items-center justify-between">
          <div>
          ...
          <div className="text-right">
            <p className={`text-xl font-bold ${cat.score >= 70 ? "text-green-500" : cat.score >= 50 ? "text-amber-500" : "text-red-500"}`}>
              {Math.round(cat.score)}
            </p>
            <Badge variant={cat.passed ? "success" : "destructive"}>{cat.passed ? "Pass" : "Fail"}</Badge>
          </div>
        </div>
      ))}
    </div>
...
export function Review({ requestId }: { requestId: string }) {
  ...
  const nextId =
    currentIndex >= 0 && currentIndex < queueIds.length - 1 ? queueIds[currentIndex + 1] : undefined;
          ...
          <Card className="xl:col-span-2">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                {faceSearchUrl && (
                  <div>
                    <p className="text-[var(--muted-foreground)] text-xs uppercase">Reference Face</p>
                    <img
                      src={faceSearchUrl}
            ...
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                {auditDetail.data?.current_agent && (
                  <p className="text-xs text-[var(--muted-foreground)]">
                    Last agent: <span className="font-medium">{auditDetail.data.current_agent}</span>
                  </p>
                )}
