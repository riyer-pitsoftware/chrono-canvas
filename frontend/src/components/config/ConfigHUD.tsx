...
export function ConfigHUD({ className }: { className?: string }) {
  ...
  const [errors, setErrors] = useState<Array<{ channel: string; error: string }>>([]);
  ...
  return (
    <div className={cn('rounded-xl border border-zinc-800 bg-zinc-950 p-4 shadow-2xl', className)}>
      {/* Header: mode toggle */}
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400">Signal Chain</h3>
        {modeLocked ? (
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Channel title="Vision & MML">
          ...
          <span className="mt-1 text-[9px] text-zinc-600">Uses LLM provider</span>
        </Channel>
        ...
        <Channel title="Compositing">
          ...
          <label className="mt-1 flex items-center gap-1.5 text-[10px] text-zinc-500">
            <input
              ...
              onChange={(e) => store.setValidationRetryEnabled(e.target.checked)}
              className="accent-amber-500"
            />
            Validation Retry
          </label>
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        {errors.length === 0 && !validating && (
          <span className="text-[10px] text-zinc-600">Config saved to localStorage</span>
        )}
      </div>
    </div>
  );
