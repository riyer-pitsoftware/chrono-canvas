...
export function ImageGallery({
  ...
  const filtered =
    filter === "all"
      ? sorted
      : sorted.filter((img) => classifyImage(img) === filter);

  const acceptedCount = sorted.filter(
    (img) => classifyImage(img) === "accepted",
  ).length;
  const rejectedCount = sorted.filter(
    (img) => classifyImage(img) === "rejected",
  ).length;
    ...
    >
          ...
          <div className="flex flex-wrap items-center justify-between w-full gap-3 text-white text-sm">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {img.validation_score != null && (
                <Badge
                  variant={
                    img.validation_score >= PASS_THRESHOLD
                      ? "success"
                      : "destructive"
                  }
                >
      ...
      {images.length > 1 && (
        ...
        >
          <div
            ref={thumbStripRef}
            className="flex gap-2 overflow-x-auto py-2 px-2 justify-center"
          >
            {images.map((thumb, i) => {
