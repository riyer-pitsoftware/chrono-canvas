...
class ComfyUIClient(ImageGenerator):
    ...
    async def _generate_and_track_progress(
        self,
        client: httpx.AsyncClient,
        workflow: dict,
        on_progress: Callable[[int, int], Awaitable[None]],
    ) -> bytes:
        """Submit prompt to ComfyUI and track sampling progress via its WebSocket.

        Connects to ComfyUI's WS *before* submitting the prompt to avoid missing
        early progress messages.  Falls back to HTTP polling on any error.
        """
        ...
        try:
            async with websockets.connect(ws_url, open_timeout=10.0) as ws:
                # Submit prompt with our client_id so ComfyUI routes WS events to us
                resp = await client.post(f"{self.base_url}/prompt", json=payload)
                resp.raise_for_status()
                prompt_id = resp.json()["prompt_id"]
                ...
                async for raw_msg in ws:
                    ...
                    # Some messages don't carry prompt_id; only filter when present
                    if msg_data.get("prompt_id") and msg_data["prompt_id"] != prompt_id:
                        continue

                    if msg_type == "progress":
                        await on_progress(
                            msg_data.get("value", 0),
                            msg_data.get("max", 1),
                        )
